...
const uploadImageToCloudinary = async (file: TImageUpload, folder = "sports-booking"): Promise<TUploadResponse> => {
  try {
    return new Promise((resolve, reject) => {
      cloudinary.uploader
        .upload_stream(
          ...
          (error, result) => {
            if (error) {
              reject(new AppError(httpStatus.BAD_REQUEST, "Image upload failed"))
            } else if (result) {
              resolve({
